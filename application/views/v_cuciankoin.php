	...
	<div class="row">
			          ...
			          <b style='padding:6px 0;display: inline-block'>
			          	<?php 
			          		if($tanggal==NULL && $jenis==0){
			          			echo "Semua Order Koin";
			          		}else if($tanggal && $jenis==0){
			          			echo "Semua Order Koin Tanggal $tanggal";
			          		}
		              ...
		              <tbody>
		              	<?php foreach($list_data->result() as $row): ?>
		              		<tr>
		              			...
		              			<td>
		              				<?php if($row->ket == 0): ?>
										<button class="btn btn-danger" onclick="dikerjakan('<?= $row->id ?>','<?= $row->invoice_no ?>')">Dikerjakan</button>
		              				<?php elseif($row->ket == 1): ?>
		              				  	<button class="btn btn-success" onclick="selesai('<?= $row->id ?>','<?= $row->invoice_no ?>')">Selesai</button>
		              				<?php elseif($row->ket == 2): ?>
		              				  	<button class='btn btn-primary'>Diambil</button>
		              				<?php endif; ?>
		              			</td>
		              			<td>
		              				<button class='btn btn-primary' onclick="print('<?= $row->invoice_no ?>')"><b class="glyphicon glyphicon-print"></b></button>
		              			</td>
		              		</tr>
		              	<?php endforeach; ?>
					  </tbody>      
	    ...
	    <div class="col-md-4">
			<div class="panel panel-primary panel-custom">
		          ...
		          <div class="panel-body">
		          	<form method="post" id="form_cari" action="<?php echo base_url('cuciankoin'); ?>">
		          	<table class="table">
		          		<tr>
		          			<td>Status</td>
							<td>
...
<?php include 'struk/btn_print-koin.php'; ?>  
	...
	function dikerjakan(id, invoice){
		event.preventDefault(); // prevent form submit
			...
			function(isConfirm){
			  if (isConfirm) {
			    $.ajax({
					url: url+'cuciankoin/selesai_koin/'+id,
					type: 'GET',
					dataType: 'html',
	...
	function selesai(id, invoice){
		event.preventDefault(); // prevent form submit
			...
			function(isConfirm){
			  if (isConfirm) {
			    $.ajax({
					url: url+'cuciankoin/ambil_koin/'+id,
					type: 'GET',
					dataType: 'html',
					success: function(pesan){
						if(pesan == "sukses"){
